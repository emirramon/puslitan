                    ...
                    <div class="card-content">
                        ...
                        <table id="datatables" class="table table-hover table-responsive">
                            ...
                            <tbody>
                                <?php $i = 1; ?>
                                <tr>
                                    <?php foreach ($artikel_model as $data) : ?>
                                        <td class="text-center"><?= $i ?></td>
                                        <td><?= $data['judul_artikel'] ?></td>
                                        <td class="td-actions">
                                            <!-- <a href="<?= $data['linkdrive'] ?>" target="_blank" rel="tooltip" class="btn btn-primary" title="Download">
                                                <i class="material-icons">cloud_download</i>
                                            </a> -->
                                            <?php if ($this->session->userdata('level') == '0' | $this->session->userdata('level') == '3') : ?>
                                                <?php if ($data['jenis_artikel'] == 0) : ?>

                                                    <a href="<?= base_url('artikel/dataabsen/' . $data['artikel_id']) ?>"><button type="button" id="edit" rel="tooltip" class="btn btn-info" title="Absen" onclick="editMateri(this.value)">
                                                            <i class="material-icons">account_box</i>
                                                        </button></a>
                                                <?php endif; ?>
                                                <a href="<?= base_url('artikel/edit/' . $data['artikel_id']) ?>"><button type="button" id="edit" rel="tooltip" class="btn btn-info" title="Edit" onclick="editMateri(this.value)">
                                                        <i class="material-icons">edit</i>
                                                    </button></a>
                                                <a href="<?= base_url() . 'Artikel/deleteartikel/' . $data['artikel_id']; ?>" rel="tooltip" class="btn btn-danger" title="Delete" onClick="if(!confirm(`Apakah Anda Yakin Menghapus <?= $data['judul_artikel'] ?> ?`)){return false;}">
							...
							<div class="modal-dialog" role="document">
								...
								<form action="<?= base_url('Download/editMateri/`+val+`'); ?>" method="post" enctype="multipart/form-data">
									<div class="modal-body">
										...
										<div class="form-group label-floating">
											<label class="control-label">Link GDrive</label>
											<input type="text" class="form-control" name="linkdrive" value="` + r[0].linkdrive + `">
										</div>
									</div>
									<div class="modal-footer">
										<button type="button" class="btn btn-secondary" data-dismiss="modal">Batal</button>
